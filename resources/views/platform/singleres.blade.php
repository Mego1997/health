        ...
        <div class="inner-banner inner-bg3">
            <div class="container">
                <div class="inner-title text-center">
                    <h3>{{$resarticles->article_name}}</h3>
                    <ul>
                        <li>
                            <a href="/homepage">الصفحة الرئيسية</a>
                        </li>
                        <li>{{$resarticles->article_name}}</li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="col-lg-8">
                        <div class="blog-details-content">
                            <div class="blog-preview-img">
                                <img src="{{ url('public/res_articles/' . $resarticles->article_image) }}" alt="Blog Images">
                            </div>
                            <ul class="tag-list">
                                <li class="active"><a href="{{ route('doctors.show', $resarticles->doc_id) }}"> <i class='bx bx-user'></i>Dr. {{$resarticles->doctor->doc_name}} </a></li>
                                <li><i class='bx bx-calendar'></i> {{$resarticles->created_at}} </li>
                                <li><i class='bx bx-comment-dots'></i> {{count($rescomments)}} تعليق </li>
                            </ul>
                            <h2>{{$resarticles->article_name}}</h2>

                            <div class="blog-article-content">
                                {!!$resarticles->article_body!!}
                            </div>
                        ...
                        <div class="side-bar-area pl-20">
                            ...
                            <div class="side-bar-widget">
                                ...
                                <div class="widget-popular-post">
                                    ...
                                    <article class="item">
                                        <a href="{{ route('platform.singleres.show', $recent->id) }}" class="thumb">
                                            <img src="{{ url('public/res_articles/' . $recent->article_image) }}" class="full-image cover bg1" role="img">

                                        </a>
                                        <div class="info">
                                            <h4 class="title-text">
                                                <a href="{{ route('platform.singleres.show', $recent->id) }}">
                                                   {{$recent->article_name}}
                                                </a>
                                            </h4>

                                            <a href="{{ route('platform.singleres.show', $recent->id) }}" class="read-btn" target="_blank">قراءة المزيد</a>
                                        </div>
                                    </article>
