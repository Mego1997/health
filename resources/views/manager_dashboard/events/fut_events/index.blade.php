                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body card-dashboard">
                                        ...
                                        <table class="table table-striped table-bordered zero-configuration">
                                            ...
                                            <tbody>
                                            @foreach ($fut_events as $key => $event)
                                            <tr>
                                                <td>{{ $key +1 }}</td>
                                                <td>{{ $event->name }}</td>
                                                <td>{!! $event->body !!}</td>
                                                <td><img src="{{ url('public/fut_events/' . $event->image) }}" width="50" height="50" alt=""></td>
                                                <td>
                                                    <a href="{{ route('manager_dashboard.events.fut_events.edit', $event->id) }}"  class="btn btn-sm btn-warning"><i class="fa-regular fa-pen-to-square"></i></a>
                                            ...
                                            <div class="modal fade text-left" id="danger{{ $event->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel10" aria-hidden="true">
                                                <form action="{{ route('manager_dashboard.events.fut_events.destroy', $event->id) }}" method="POST">
                                                    @csrf
                                                    @method('delete')
                                            ...
                                            <a class="btn btn-outline-primary btn-lg" href="{{ asset('/manager/dashboard/fut_events/create') }}">
                                                اضافة فعالية
                                            </a>
                                    </div>
                                </div>
