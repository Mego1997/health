        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-6 col-12 mb-2">
                    <h3 class="content-header-title mb-0">المقالات</h3>
                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body card-dashboard">
                                        ...
                                        <table class="table table-striped table-bordered zero-configuration">
                                            ...
                                            <tbody>
                                            @foreach ($info_articles as $key => $article)
                                            <tr>
                                                <td>{{ $key +1 }}</td>
                                                <td>{{ $article->article_name }}</td>
                                                <td><img src="{{ url('public/info_articles/' . $article->article_image) }}" width="50" height="50" alt=""></td>
                                                <td>{{ $article->He_Info_Category->cat_name}}</td>
                                                <td>{{ $article->He_Info_Tag->tag_name}}</td>
                                                <td>{{ $article->doctor->doc_name}}</td>
                                                <td>
                                                    <a href="{{ route('manager_dashboard.healthy.he_info_articles.edit', $article->id) }}"  class="btn btn-sm btn-warning"><i class="fa-regular fa-pen-to-square"></i></a>
                                            ...
                                            <div class="modal fade text-left" id="danger{{ $article->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel10" aria-hidden="true">
                                                <form action="{{ route('manager_dashboard.healthy.he_info_articles.destroy', $article->id) }}" method="POST">
                                                    @csrf
                                                    @method('delete')
                                            ...
                                            <a class="btn btn-outline-primary btn-lg" href="{{ asset('/manager/dashboard/info_articles/create') }}">
                                                اضافة مقالة
                                            </a>
                                    </div>
                                </div>
